...
// Private to this module: getTsRefineFormat is the only entry point.
function buildFormatTokens(options: TSR.FormatStyle): string[] {
    const flags: string[] = []
...
// Renders a FormatStyle as the flag string the `format` command consumes.
// Takes FormatStyle (not ReportResult) so the format command can pass its
// override-merged style straight through; report callers convert first via
// reportToFormatStyle. Returns plain text (empty when nothing fired), so
// the caller picks its own framing.
export function getTsRefineFormat(options: TSR.FormatStyle): string {
    return buildFormatTokens(options).join(" ")
}
...
export function writeFormatCommand(report: TSR.ReportResult, output: TSR.Writer): void {
    const format = getTsRefineFormat(reportToFormatStyle(report))
    if (!format) {
