...
// the value writeFormatCommand frames and the Markdown survey embeds. Takes
// the FormatStyle rather than the report so the `format` runner can render
// its merged survey+override style. Returns plain text (empty when the
// style carries nothing), so the caller picks its own framing.
export function getTsRefineFormat(format: TSR.FormatStyle): string {
    return buildFormatTokens(format).join(" ")
}
