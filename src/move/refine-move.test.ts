...
describe("refineMove (in-memory, dry-run)", () => {
    ...
    it("preserves the mandatory `.json` extension when moving a file that imports JSON", async () => {
        ...
        await refineMove({project, log, sources: ["/src/main.ts"], dest: "/src/sub/", dryRun: true})
    ...
    it("preserves the dynamic-import extension across the NodeNext js↔ts mapping (.mjs → .mts source)", async () => {
        ...
        await refineMove({project, log, sources: ["/src/a.mts"], dest: "/src/sub/", dryRun: true})
    ...
    it("treats a trailing-slash dest as a directory move when multiple sources are given", async () => {
        ...
        const result = await refineMove({project, log, sources: ["/src/a.ts", "/src/b.ts"], dest: "/lib/", dryRun: true})
    ...
    it("treats a single-source non-directory dest as a rename", async () => {
        const project = newProject()
        project.createSourceFile("/src/a.ts", "export const x = 1\n")
        const result = await refineMove({project, log, sources: ["/src/a.ts"], dest: "/src/renamed.ts", dryRun: true})
    ...
    it("rejects multi-source moves into a non-directory dest", async () => {
        ...
        project.createSourceFile("/src/a.ts", "export const x = 1\n")
        project.createSourceFile("/src/b.ts", "export const y = 2\n")
        await assert.rejects(() => refineMove({project, log, sources: ["/src/a.ts", "/src/b.ts"], dest: "/src/target.ts", dryRun: true}), /destination must be an existing directory/)
    })

    it("rejects a source that is not in the project", async () => {
        const project = newProject()
        await assert.rejects(() => refineMove({project, log, sources: ["/src/missing.ts"], dest: "/src/sub/", dryRun: true}), /not in the project/)
    })

    it("rejects when source and destination are the same path", async () => {
        const project = newProject()
        project.createSourceFile("/src/a.ts", "export const x = 1\n")
        await assert.rejects(() => refineMove({project, log, sources: ["/src/a.ts"], dest: "/src/a.ts", dryRun: true}), /source and destination are the same/)
    })
    ...
    it("rejects when destination is an existing project file that isn't being moved out", async () => {
        ...
        project.createSourceFile("/src/a.ts", "export const x = 1\n")
        project.createSourceFile("/src/b.ts", "export const y = 2\n")
        await assert.rejects(() => refineMove({project, log, sources: ["/src/a.ts"], dest: "/src/b.ts", dryRun: true}), /destination already exists/)
    })
    ...
    it("rejects when destination is on the project FS but excluded from the tsconfig", async () => {
        ...
        await assert.rejects(() => refineMove({project, log, sources: ["/src/a.ts"], dest: "/dist/precious.ts", dryRun: true}), /destination already exists/)
    })
    ...
    it("resolves dynamic imports that target a directory's index file", async () => {
        ...
        const result = await refineMove({project, log, sources: ["/src/feature/index.ts"], dest: "/src/sub/index.ts", dryRun: true})
    ...
    it("recognizes in-memory directories without a trailing slash (project FS, not host FS)", async () => {
        ...
        const result = await refineMove({project, log, sources: ["/src/a.ts"], dest: "/lib", dryRun: true})
    ...
    it("re-sorts the import block of an import-changed file", async () => {
        ...
        await refineMove({project, log, sources: ["/src/m.ts"], dest: "/src/sub/", dryRun: true})
    ...
    it("keeps the importer's spaced braces (surveyed per file) while sorting", async () => {
        const project = newProject()
        project.createSourceFile("/src/a.ts", "export const x = 1\nexport const w = 2\n")

        // The file wrote `{ x, w }` (spaced); surveyed on its own, it stays spaced.
    ...
    it("keeps the importer's tight braces (surveyed per file) while sorting", async () => {
        const project = newProject()
        project.createSourceFile("/src/a.ts", "export const x = 1\nexport const w = 2\n")

        // The file wrote `{x,w}` (no space); surveyed on its own, it stays tight.
    ...
    it("organizes each changed file in its own surveyed style (moved file + importer)", async () => {
        const project = newProject()
        project.createSourceFile("/src/dep.ts", "export const y = 2\n")

        // The moved file already uses spaced braces; its importer uses tight
        // braces. Each is surveyed on its own, so both keep their style after the
        // move re-sorts their (repathed) imports.
        project.createSourceFile("/src/a.ts", 'import { y } from "./dep.ts"\nexport const x = y\n')
        const b = project.createSourceFile("/src/b.ts", 'import {x} from "./a.ts"\nconst _ = x\n')
        await refineMove({project, log, sources: ["/src/a.ts"], dest: "/src/sub/", dryRun: true})

        // The moved file is surveyed at its pre-move state (keyed by node), so its
        // own import keeps the spaced style; the importer keeps its tight style.
...
describe("refineMove (on disk)", () => {
    ...
    it("does not persist unrelated pending edits on the project (selective save)", async () => {
        ...
        try {
            ...
            await refineMove({project, log, sources: [path.join(sub, "src/a.ts")], dest: path.join(sub, "lib/"), dryRun: false})
    ...
    it("persists the move and the importer rewrite to disk", async () => {
        ...
        await refineMove({project, log, sources: [path.join(workdir, "src/a.ts")], dest: path.join(workdir, "lib/"), dryRun: false})
