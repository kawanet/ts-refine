...
describe("refineFormat", () => {
    it("applies the indent width from the format style", async () => {
        ...
        await refineFormat({project, log, dryRun: true, paths: [], style: {indent: 4}})
    ...
    it("applies a pinned indent width", async () => {
        const project = initInMemoryProject()
        const sf = project.createSourceFile("a.ts", "function f() {\n  return 1\n}\n")
        await refineFormat({project, log, dryRun: true, paths: [], style: {indent: 2}})
        assert.match(sf.getFullText(), /\n {2}return 1\n/)
    })
    ...
    it("inserts trailing semicolons when format.semicolons is 'on'", async () => {
        ...
        await refineFormat({project, log, dryRun: true, paths: [], style: {semi: "on"}})
    ...
    it("keeps bracketSpacing off symmetric for single-line type literals", async () => {
        const style = {
            semi: "on",
            indent: 4,
        ...
        const cases = [
            ...
            ["type Option = { isSP: boolean, query: Query };\n", "type Option = {isSP: boolean, query: Query};\n"],
            ["const ret = await getJSON<{ data: T }>(endpoint, query);\n", "const ret = await getJSON<{data: T}>(endpoint, query);\n"],
            ["const index = {} as { [key: string]: Render };\n", "const index = {} as {[key: string]: Render};\n"],
        ]
    ...
    it("trims only the last single-line type literal member across delimiter styles", async () => {
        for (const memberDelimiter of [undefined, "semi", "comma", "none"] as const) {
            ...
            await refineFormat({project, log, dryRun: true, paths: [], style: {semi: "on", memberDelimiter}})
    ...
    it("strips trailing semicolons when format.semicolons is 'off'", async () => {
        const project = initInMemoryProject()
        const sf = project.createSourceFile("a.ts", "const a = 1;\nconst b = 2;\n")
        await refineFormat({project, log, dryRun: true, paths: [], style: {semi: "off"}})
        assert.match(sf.getFullText(), /const a = 1\nconst b = 2\n/)
    })
    ...
    it("formats .d.ts files too (no longer excluded)", async () => {
        ...
        await refineFormat({project, log, dryRun: true, paths: [], style: {bracketSpacing: "on"}})
    ...
    it("returns the touched files, and an empty list when nothing changes", async () => {
        const project = initInMemoryProject()
        const sf = project.createSourceFile("a.ts", "const a = 1\n")
        const changed = await refineFormat({project, log, dryRun: true, paths: [], style: {semi: "on"}})
        assert.deepEqual(changed.touched, [sf.getFilePath()])

        // The same pass over the now-formatted in-memory state changes nothing.
        const again = await refineFormat({project, log, dryRun: true, paths: [], style: {semi: "on"}})
